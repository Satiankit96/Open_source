...
class FinBERTSummarizer:
    def __init__(self, model_name: str = "yiyanghkust/finbert-tone", chunk_size: int = 512, max_summary_length: int = 300):
        """
        Initialize the text summarizer with the FinBERT model.
        :param model_name: Name of the FinBERT model to use for sentiment-guided summarization.
        :param chunk_size: The maximum size of each chunk for summarization (FinBERT's max token size is 512).
        :param max_summary_length: The maximum length of the summary to keep it concise.
        """
    ...
    def chunk_text(self, text: str) -> list:
        """
        Split the text into manageable chunks for sentiment analysis based on the tokenizer's max length.
    ...
    def analyze_sentiment(self, text: str) -> str:
        """
        Use FinBERT to analyze sentiment of the given text (entire summary in this case).
        :param text: The entire summarized text.
        :return: The sentiment label for the summarized text (Positive, Negative, Neutral).
        """
    ...
    def summarize_chunks(self, chunks: list) -> str:
        """
        Summarize the text chunks by identifying key chunks based on their sentiment.
    ...
    def truncate_summary(self, summary: str) -> str:
        """
        Truncate the summary to the specified maximum length to keep it concise.
        :param summary: The full summary.
        :return: A truncated summary.
        """
        tokens = summary.split()
        if len(tokens) > self.max_summary_length:
            return " ".join(tokens[:self.max_summary_length]) + "..."
        return summary

    def summarize_text(self, text: str) -> str:
        """
        Generate a concise summary for the provided text using FinBERT.
        ...
        combined_summary = self.summarize_chunks(chunks)

        logging.info("Summary generated successfully.")
        return combined_summary

    def summarize_and_analyze(self, text: str) -> str:
        """
        Summarize the input text and then analyze the sentiment of the entire summary.
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler("finbert_summarizer.log"),
