...
def chunk_sentences(sentences, chunk_size):
    """
    Splits the list of sentences into chunks of the specified size.
    ...
    Returns:
        list: A list of chunks, each containing a subset of sentences.
    """
    for i in range(0, len(sentences), chunk_size):
        yield sentences[i:i + chunk_size]

def build_similarity_matrix(sentences, vectorizer=None):
    """
    Builds a cosine similarity matrix for the given sentences.

    Parameters:
        sentences (list): A list of sentences.
        vectorizer (TfidfVectorizer): Pre-configured TF-IDF vectorizer for reuse.

    Returns:
        np.ndarray: A matrix representing cosine similarities between sentences.
    """
...
def rank_sentences(similarity_matrix):
    """
    Ranks sentences using PageRank on the similarity matrix.

    Parameters:
        similarity_matrix (np.ndarray): The similarity matrix.

    Returns:
        dict: A dictionary of sentence indices and their corresponding PageRank scores.
    """
    nx_graph = nx.from_numpy_array(similarity_matrix)
    return nx.pagerank(nx_graph)

def summarize_chunk(chunk, num_sentences=3):
    """
    Summarizes a chunk of sentences using the TextRank algorithm.

    Parameters:
        chunk (list): A list of sentences in the chunk.
        num_sentences (int): The number of sentences to include in the summary.

    Returns:
        str: The summarized text for the chunk.
    """
    similarity_matrix = build_similarity_matrix(chunk)
    sentence_ranks = rank_sentences(similarity_matrix)

    ranked_sentences = sorted(((rank, idx) for idx, rank in sentence_ranks.items()), reverse=True)
    selected_sentences = sorted([chunk[idx] for rank, idx in ranked_sentences[:num_sentences]])

    return ' '.join(selected_sentences)

def summarize_text(text, num_sentences=3, chunk_size=5):
    """
    Summarizes the input text using chunking and an optimized TextRank algorithm.

    Parameters:
        text (str): The input text to summarize.
        num_sentences (int): The number of sentences to include in each chunk's summary.
...
summary = summarize_text(text, num_sentences=2, chunk_size=3)
print("Summary:\n", summary)
