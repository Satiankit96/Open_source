...
# Suppress warnings related to uninitialized model weights
...
class BARTSummarizer:
    ...
    def summarize_chunks(self, chunks: list, prompt: str, max_length: int = 350, min_length: int = 150) -> str:
        ...
        for chunk in chunks:
            ...
            summary_ids = self.model.generate(
                ...
                length_penalty=2.0,  # Adjusted length penalty
                repetition_penalty=1.8,  # Adjusted repetition penalty
                num_beams=10,  # Increased number of beams for diversity
                temperature=0.5,
                do_sample=True,
                early_stopping=True
            )
    ...
    def calculate_metrics(self, generated_summary: str, ground_truth: str):
        """
        Calculate ROUGE, METEOR, SacreBLEU, BERTScore, and BLEU score between the generated summary and the ground truth.
        :param generated_summary: The generated summary text.
        :param ground_truth: The ground truth text.
        :return: A dictionary containing the calculated scores.
        """
        # Tokenize the ground truth and generated summary for METEOR and BLEU
        ...
        # METEOR score
        meteor = meteor_score([ground_truth_tokens], generated_summary_tokens)
        ...
        bleu = sentence_bleu([ground_truth_tokens], generated_summary_tokens, smoothing_function=smoothie)

        # BERTScore
        P, R, F1 = bert_score([generated_summary], [ground_truth], lang="en", model_type="roberta-large")
...
def main():
    """
    Main function to execute the summarizer and calculate the scores.
    """
    setup_logging()

    # Example usage with a text to summarize and the ground truth for comparison
    article_text = """
...
"""
    ...
    try:
        ...
        prompt = "Summarize the article from the perspective of a risk manager at a global bank: "

        # Generate the cumulative summary based on the prompt
        generated_summary = summarizer.summarize_text(article_text, prompt)
        print("Generated Summary:")
        print(generated_summary)

        # Calculate metrics against the ground truth
        scores = summarizer.calculate_metrics(generated_summary, ground_truth)
        ...
        logging.error(f"Error during summarization or scoring: {e}")
